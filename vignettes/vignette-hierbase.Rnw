...
If you use the \texttt{hierbase} package, please cite the paper Renaux, C., and B\"uhlmann, P. (2021). Efficient Multiple Testing Adjustment for Hierarchical Inference.
\textit{arXiv preprint arXiv:2104.15028}.
...
the same motivation by \cite{Meins08} and further refined by subsequent
work. It is a data-driven multiple testing approach to find significant groups
...
at each depth of the hierarchical tree. Depth-wise Bonferroni has been
...
Our \textsf{R} package \texttt{hierbase} implements our SBH inheritance rule as well 
as depth-wise Bonferroni. We demonstrate the function 
...
test function \texttt{QF} used in this example from the \textsf{R} package \texttt{SIHR}. 
